...
# Botón de arrepentimiento (cancelación dentro de 60 segundos desde la venta)
def anular_venta_reciente():
    ...
    if tiempo_transcurrido <= 60:  # 60 segundos = 1 día en esta simulación
        if estado_actual == 3:
            print("La venta ya está cancelada.")
        else:
            cursor.execute("UPDATE venta SET ID_Estado_Venta = 3, Fecha_anulacion = %s WHERE ID_Venta = %s", (ahora, id_venta))
            conexion.commit()
            print("Venta anulada correctamente dentro del tiempo permitido.")
    else:
        print("Ya pasó el plazo para cancelar esta venta.")
